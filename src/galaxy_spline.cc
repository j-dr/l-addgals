...
FiveTuple denspdf_params(float magnitude, float zred)
{
  ...
  fm = fm0 + fm1*mag + fm2*mag*mag + fmz*zred;
...
FiveTuple denspdf_params_spline(float magnitude, float zred, denspdf PDF)
{
  //just call the splines that have already been built
  float cm = PDF.cm(zred, magnitude);
  float cs = PDF.cs(zred, magnitude);
  float fm = PDF.fm(zred, magnitude);
  float fs = PDF.fs(zred, magnitude);
  float p = PDF.p(zred, magnitude);
...
*/
den_ent define_prob(float magnitude, float redshift, float vol, denspdf PDF)
{
  ...
  FiveTuple fp = denspdf_params_spline(magnitude+dmag, redshift, PDF);
  FiveTuple fpp = denspdf_params_spline(magnitude-dmag, redshift, PDF);
...
float LocalDens(den_ent pdf)
{
  ...
  int nbin = NBIN;
  float d8 = -1.0;
  ///for(int ind=0;ind<nbin;ind++) cout<<pdf.r[ind]<<" "<<pdf.prob[ind]<<endl;
  while(d8 < d8_min || d8 > d8_max){
    ...
    //cout<<d8_min<<" "<<d8<<" "<<dx<<" "<<slope<<" "<<ind<<" "<<pdf.r[ind-1]<<" "<<pdf.prob[ind-1]<<" "<<d8_max<<endl;
  }
...
float LocalDens(float magnitude, float redshift, float vol, denspdf PDF)
{

  //define our probability distribution for this galaxy
  //cout<<"Defining probability array..."<<endl;
  den_ent pdf = define_prob(magnitude, redshift, vol, PDF);
  //for(int i=0;i<NBIN;i++){
...
vector <Galaxy *> GetGalaxies(double vol, float phi_rescale){
  ...
  //initialize our denspdf splines
  denspdf PDF;
  PDF.initalize(denspdffile);
  ...
  for(double mag=-23.7; mag <=-8; mag+=0.001){
	...
	if (tmag > -19.0) tmag = -19.0;
        pdfarray[iz] = define_prob(tmag,tz,vol, PDF);
      }
    ...
    for(int i=0;i<diff;i++){
      ...
      int pdf_zbin = alglib::round((double) (zGal-ZREDMIN)/dz_pdf);
      ...
      float dist8 = 0.1;
      if (mag > -23.0) LocalDens(pdfarray[pdf_zbin]);
