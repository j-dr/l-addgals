...
vector <Halo*> ReadHVHalos(void){
...
#endif
#ifdef MS_Gas //KLUDGE!!!!!!!
    filename = "/nfs/slac/g/ki/ki01/mbusha/data/hubble/PO/lcdm.POW.msort22";
#endif
  ifstream file(filename.c_str());
  if (file.fail()) {
    cerr<<"[read_cube error]: cannot open "<<filename<<endl;
    exit(1);
  } else {
    cout<<"[read_cube]: opened file "<<filename<<" for reading halo information."<<endl;
  }
  ...
  while(file){
    ...
    Point pos(xx, yy, zz);
    float ra = angle_const*atan2(yy,xx);
    if(ra<0)
      ra +=360.; 
    float dec = angle_const*asin(zz/sqrt(xx*xx + yy*yy + zz*zz));
    float Zred = cosmo.ZofR(sqrt(xx*xx + yy*yy + zz*zz)); 
    if (ra < ramin) ramin = ra;
    if (ra > ramax) ramax = ra;
    if (dec < decmin) decmin = dec;
    if (dec > decmax) decmax = dec;
    if (Zred < zrmin) zrmin = Zred;
    if (Zred > zrmax) zrmax = Zred;
    //if (ra <= RAMAX && ra >= RAMIN && dec <= DECMAX && dec>=DECMIN && Zred <= ZREDMAX && Zred >= ZREDMIN){
  ...
  cout<<"min/max Zred = "<<zrmin<<" "<<zrmax<<endl;
  return halos;
}
