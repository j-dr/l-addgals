pro create_catalog, name, gals, halos, outname=outname, fixa=fixa, $
		    ...
		    euclid=euclid, irac=irac, wise=wise, wfirst=wfirst, $
                    twomass=twomass, galz=galz, candels=candels, cut=cut ;noswap=noswap
...
IF NOT keyword_set(candels) then candels=0
...
print, 'creating colors'
  if not keyword_set(nocolor) then assign_colors, path=path, matches=matches,$ 
    ...
    wise=wise, candels=candels, cut=cut
...
new_file_reader, gals, pathname=path, des=des, vista=vista, deep=deep,$
	         ...
	         euclid=euclid, wfirst=wfirst, twomass=twomass, irac=irac, $
                 wise=wise, candels=candels, galz=galz, cut=cut
...
IF (keyword_set(des)) then begin
  ...
  gals_des1 = create_struct('omag',fltarr(6), 'amag',fltarr(6))
...
IF keyword_set(candels) then begin
  candelsname=name+'_candels'
  IF keyword_set(dim) then candelsname += '_dim'
  candelsname += '.fit'
  IF keyword_set(cut) then candelsname+='.cut'
  gals_candels1 = create_struct('omag',fltarr(10), 'amag',fltarr(10))
  gals_candels = replicate(gals_candels1,N_ELEMENTS(gals))
  gals_candels.omag = gals.ocandelsmag
  gals_candels.amag = gals.acandelsmag
  mwrfits, gals_candels, candelsname, /create
ENDIF
