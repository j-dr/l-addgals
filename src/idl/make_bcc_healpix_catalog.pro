pro make_bcc_healpix_catalog,path, outpath, healpix_num, hfile, mag_lim=mag_lim, no_lensing=no_lensing, boxsize=boxsize, llbins=llbins, zmax=zmax, zmin=zmin, zpadding=zpadding
  ...
  sfx = healpix_num+'.fit'
  gfiles = list_with_path('*galaxies.'+sfx, path)
  desfiles = list_with_path('*des.'+sfx, path)
  vistafiles = list_with_path('*vista.'+sfx, path)
  deepfiles = list_with_path('*deep.'+sfx, path)
  johnsonfiles = list_with_path('*johnson.'+sfx, path)
  flamexfiles = list_with_path('*flamex.'+sfx, path)
 
  n_files = N_ELEMENTS(gfiles)

  g = mrdfits(gfiles[0], 1)
  des = mrdfits(desfiles[0],1)
  vista = mrdfits(vistafiles[0],1)
  deep = mrdfits(deepfiles[0],1)
  johnson = mrdfits(johnsonfiles[0],1)
  flamex = mrdfits(flamexfiles[0],1)
  tzrange = get_aardvark_redshift_range(gfiles[0])
  min0 = min(tzrange[0])
  max0 = max(tzrange[1])
  ii = where(g.z ge min0 and g.z le max0,count)
...
;    lg = mrdfits(path+'/from_joerg/galaxies_full_2.fit',1)
  ...
  for i = 1, n_files - 1 do begin
    ...
    tdes = mrdfits(desfiles[i],1)
    tvista = mrdfits(vistafiles[i],1)
    tdeep = mrdfits(deepfiles[i],1)
    tjohnson = mrdfits(johnsonfiles[i],1)
    tflamex = mrdfits(flamexfiles[i],1)
    tzrange = get_aardvark_redshift_range(gfiles[i])
    min0 = min(tzrange[0])
    max0 = max(tzrange[1])
    ii = where(tg.z ge min0 and tg.z le max0, count)
    print, 'length(ii): ', size(ii)
    if (count eq 0) then continue
    g = [g,tg[ii]]
    des = [des, tdes[ii]]
    vista = [vista, tvista[ii]]
    deep = [deep, tdeep[ii]]
    johnson = [johnson, tjohnson[ii]]
    flamex = [flamex, tflamex[ii]]
  endfor
  print, 'done reading catalogs, now selecting using ii'
  ...
  add_to_hod, h, obs_full, boxsize=boxsize, llbins=llbins
...
;  endfor
  ...
  mwrfits, obs_full, outpath+'_truth_no_photoz.'+healpix_num+'.fit', hdr_t, /create
  mwrfits, obs_visible, outpath+'_no_photoz.'+healpix_num+'.fit', hdr_v, /create
  mwrfits, sdss_mag, outpath+'_sdss_mag.'+healpix_num+'.fit', hdr_sdss, /create
  mwrfits, vista_mag, outpath+'_vista_mag.'+healpix_num+'.fit', hdr_vista, /create
  mwrfits, johnson_mag, outpath+'_johnson_mag.'+healpix_num+'.fit', hdr_johnson, /create
  mwrfits, deep_mag, outpath+'_deep_mag.'+healpix_num+'.fit', hdr_deep, /create
  mwrfits, flamex_mag, outpath+'_flamex_mag.'+healpix_num+'.fit', hdr_deep, /create
;  mwrfits, halos, outpath+'_halos.'+healpix_num+'.fit', /create
