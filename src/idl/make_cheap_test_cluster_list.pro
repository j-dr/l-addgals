...
for i = 0L, ng - 1 do begin
  ...
  if (g(i).haloid gt -0.5) then begin
    if (ran(i) lt mismatch) then begin
      num = floor(random(i)*n_in_bin(dbin))
      gout(i).haloID = g(ih(dbin,num)).haloID
      if (ran(i) lt double) then begin
        num += 2    
        if (num ge n_in_bin(dbin)) then num -= 3
        gout(i).haloID2 = g(ih(dbin,num)).haloID
      endif
    endif else begin
      g(i).haloID = -1
    endelse
...
openw,1,'~/ki01/projects/addgals/PO_oct/v1.07_z/idl/test_clusters_slice_perfect.ascii'
for i = 0L, N_ELEMENTS(ii) - 1 do begin
  ih = ii(srt(i))
...
openw,1,'~/ki01/projects/addgals/PO_oct/v1.07_z/idl/test_galaxies_slice_perfect.ascii'
for i = 0L, ngout - 1 do begin
