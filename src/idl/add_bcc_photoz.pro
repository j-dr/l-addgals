pro add_bcc_photoz,path
  ...
  obs_full = mrdfits(path+'DES_Mock_Baseline_truth_no_photoz.fit', 1)
  obs_visible = mrdfits(path+'DES_Mock_Baseline_no_photoz.fit', 1)
...
;  train_arborz_photoz, training_file
  ...
  for i = 0, 9 do begin
     ...
     mwrfits, obs_visible[ii], path+'/DES_Mock_Baseline.'+strcompress(string(i),/remove_all)+'.fit', /create
     mwrfits, obs_full[ii], path+'/DES_Mock_Baseline_truth.'+strcompress(string(i),/remove_all)+'.fit', /create
  endfor

  mwrfits, obs_full, path+'DES_Mock_Baseline_truth.fit', /create
  mwrfits, obs_visible, path+'DES_Mock_Baseline.fit', /create
