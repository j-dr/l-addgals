...
void define_prob(vector <den_ent *> &pdf, vector <int> &weight, vector <FiveTuple> &denspdf, vector <float> &dmagbins, float vol)
{
  ...
  for(int id=0;id<denspdf.size()-1;id++){
    ...
    for(int i=0;i<nbin;i++){
      pdf[id]->r[i] = (i+1)*rdel;
      float p1 = 0.5*(1. - fp[4])*(1+erf((log(pdf[id]->r[i])-fp[0])/(fp[1]*sqrt(2.0))));
      float p2 = 0.5*fp[4]*(1+erf((pdf[id]->r[i]-fp[2])/(fp[3]*sqrt(2.0))));
      float p3 = 0.5*(1. - fpp[4])*(1+erf((log(pdf[id]->r[i])-fpp[0])/(fpp[1]*sqrt(2.0))));
      float p4 = 0.5*fpp[4]*(1+erf((pdf[id]->r[i]-fpp[2])/(fpp[3]*sqrt(2.0))));
      pdf[id]->prob[i] = weight1*(p1+p2) - weight2*(p3+p4);
    }

    //renormalize our probability distribution so that it integrates to unity
    float norm = pdf[id]->prob[nbin-1];
    if (norm > 0){
      for(int i=0;i<nbin;i++)
	pdf[id]->prob[i] /= norm;
    }
...
void set_d8(vector <Galaxy *> &galaxies, int idenspdf){
  ...
  ReadPDFs(denspdf, dmagbins); 
  assert(denspdf.size()== dmagbins.size());
  FiveTuple fTup = denspdf[idenspdf];

  for(int i=0;i<galaxies.size();i++){
    float dist8 = fTup.LocalDens();
    galaxies[i]->Dist8(dist8);
  }
}
