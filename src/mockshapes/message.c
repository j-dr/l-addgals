...
*/
void
msg_init(int verbosity, char *logfile)
{
...
msg_state *
msg_set_get_state(int set, int verbosity, char *logfile)
{
	...
	omp_set_lock(&l);
        ...
        if (!set) {
            ...
            msg_message(MSG_WARNING, 0,
                        "Trying to use message interface without initializing it. Continuing with default settings.");
        }
        if (logfile) {
...
void
msg_message(int severity, int overwrite, char *format, ...)
{
#pragma omp critical
    {
        msg_state *state;
        ...
        if (severity <= state->verbosity) {
            ...
            if (overwrite == 1)
                ...
                fputc('\n', stream);
            fprintf(stream, "[%s%9s] %s", msg_severity[severity], buf, msg);
            if (overwrite != 1)
                fputc('\n', stream);
            else
                fflush(stream);
            if (state->f)
                fprintf(state->f, "[%s%9s] %s\n", msg_severity[severity],
                        buf, msg);
        }
    }
}
...
*/
void
msg_format_time(char *buf, float t)
{
